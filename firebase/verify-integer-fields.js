...
async function checkCollection(collectionName) {
  ...
  for (const field of INTEGER_FIELDS) {
    console.log(`${field}:`);
    console.log(`  ✅ int: ${typeStats[field].int}`);
    console.log(`  ⚠️  double: ${typeStats[field].double}`);
    console.log(`  ❌ string: ${typeStats[field].string}`);
  ...
  if (allIssues.length > 0) {
    ...
    console.log(`✅ 沒有發現問題! 所有整數欄位都是正確格式\n`);
  }
...
async function main() {
  console.log(`\n╔════════════════════════════════════════╗`);
  console.log(`║  Firestore 整數欄位驗證腳本            ║`);
  console.log(`╚════════════════════════════════════════╝\n`);
  ...
  if (totalIssues === 0) {
    ...
    console.log(`\n執行以下命令進行修復:`);
    console.log(`  node firebase/migrate-integer-fields.js`);
  }
