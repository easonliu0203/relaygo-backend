 ...
 * Firestore 時間戳欄位遷移腳本
 * 
 * 功能: 將 String 格式的時間戳轉換為 Firestore Timestamp 格式
 * 
 * 背景:
 * - 舊版 Edge Function 將時間戳存儲為 String
 * - 新版 Edge Function 使用正確的 Timestamp 格式
 * - 客戶端期望 Timestamp 格式,遇到 String 會報錯
 * 
 * 使用方法:
 * 1. 安裝依賴: npm install firebase-admin
 * 2. 設置環境變數: export GOOGLE_APPLICATION_CREDENTIALS="path/to/serviceAccountKey.json"
 * 3. 運行腳本: node firebase/migrate-timestamp-fields.js
 */
...
// 需要遷移的時間戳欄位
const TIMESTAMP_FIELDS = ['bookingTime', 'createdAt', 'matchedAt', 'completedAt'];

/**
 * 檢查欄位是否是 String 格式的時間戳
 */
function isStringTimestamp(value) {
...
/**
 * 將 String 轉換為 Firestore Timestamp
 */
...
async function migrateDocument(collectionName, docRef) {
  try {
    ...
    const data = doc.data();
    const updates = {};
    let hasStringTimestamp = false;
    
    // 檢查每個時間戳欄位
    for (const field of TIMESTAMP_FIELDS) {
      if (data[field] && isStringTimestamp(data[field])) {
        hasStringTimestamp = true;
        updates[field] = convertToTimestamp(data[field]);
        console.log(`  🔄 ${field}: "${data[field]}" → Timestamp`);
      }
    }
    
    // 如果有需要更新的欄位,執行更新
    if (hasStringTimestamp) {
...
async function verifyMigration(collectionName) {
  ...
  if (issues.length === 0) {
    console.log(`✅ 驗證通過: 所有時間戳欄位都是正確格式`);
  } else {
    console.log(`⚠️  發現 ${issues.length} 個問題:`);
    issues.forEach(issue => {
      console.log(`  - ${issue.docId}.${issue.field}: "${issue.value}"`);
    });
  }
...
async function main() {
  console.log(`\n╔════════════════════════════════════════╗`);
  console.log(`║  Firestore 時間戳欄位遷移腳本          ║`);
  console.log(`╚════════════════════════════════════════╝\n`);
  
  console.log(`📋 遷移計劃:`);
  console.log(`  集合: ${COLLECTIONS.join(', ')}`);
  console.log(`  欄位: ${TIMESTAMP_FIELDS.join(', ')}`);
  console.log(`  操作: String → Firestore Timestamp\n`);
  ...
  // await new Promise(resolve => rl.question('繼續? (yes/no): ', answer => { rl.close(); resolve(answer); }));
  ...
  if (totalIssues === 0) {
    console.log(`🎉 遷移成功! 所有時間戳欄位都已轉換為正確格式`);
  } else {
