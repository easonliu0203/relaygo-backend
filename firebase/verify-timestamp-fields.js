...
async function checkDocument(collectionName, doc) {
  ...
  for (const field of TIMESTAMP_FIELDS) {
    ...
    // 如果不是 Timestamp 或 null,記錄為問題
    if (type !== 'Timestamp' && type !== 'null') {
      issues.push({
        docId: doc.id,
        field: field,
        type: type,
...
async function checkCollection(collectionName) {
  ...
  for (const field of TIMESTAMP_FIELDS) {
    console.log(`${field}:`);
    console.log(`  ✅ Timestamp: ${typeStats[field].Timestamp}`);
    console.log(`  ⚠️  String (valid): ${typeStats[field]['String (valid date)']}`);
    console.log(`  ❌ String (invalid): ${typeStats[field]['String (invalid date)']}`);
  ...
  if (allIssues.length > 0) {
    ...
    for (const field of TIMESTAMP_FIELDS) {
      if (issuesByField[field] && issuesByField[field].length > 0) {
        ...
        for (let i = 0; i < samplesToShow; i++) {
          ...
          console.log(`  - ${issue.docId}: ${issue.type} = "${issue.value}"`);
        }
    ...
    console.log(`✅ 沒有發現問題! 所有時間戳欄位都是正確格式\n`);
  }
...
async function main() {
  console.log(`\n╔════════════════════════════════════════╗`);
  console.log(`║  Firestore 時間戳欄位驗證腳本          ║`);
  console.log(`╚════════════════════════════════════════╝\n`);
  ...
  if (totalIssues === 0) {
    ...
    console.log(`\n執行以下命令進行修復:`);
    console.log(`  node firebase/migrate-timestamp-fields.js`);
  }
